<?php 
	...
	if(isset($samasyesreni['atadaaidi'])){
		$alreadySettled = mysqli_query($conn, "SELECT 1 FROM `gellaluhogiondu_kemeru_phalitansa_drei` WHERE `kalaparichaya` = '".$samasyesreni['atadaaidi']."' LIMIT 1");
		if ($alreadySettled && mysqli_num_rows($alreadySettled) > 0) {
			return;
		}
		...
		if($gadhipathulidhadi >= 1){
			...
			if (!function_exists('sort_with_positions')) {
			...
			if($kouta == 0){
				...
				if (!function_exists('generateUniqueDigitNumber')) {
			...
			if($kouta == 1){
				...
				if (!function_exists('find_third_digit')) {
			...
			if($kouta == 3){
				$totalsum = $realnumber;
				if (!function_exists('findThreeDigitNumbers')) {
			...
			if (!function_exists('checkNumberProperties')) {
